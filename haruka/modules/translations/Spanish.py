...
MARKDOWN_HELP = """
    ...
    - <code>*negrita*</code>
    - <code>`codigo`</code>
    - <code>[algúntexto](algunaURL)</code>: esto creará un link \

    EJ: <code>[test](example.com)</code>

    - <code>[textodelboton](buttonurl:algunaURL)</code>
...
#__main__
    #Module names
        ...
        "AFK": "Ausente (AFK)",
        "Domain Blacklists": "Urls prohibidas",
        "AntiFlood": "Antiflood",
        "Bans": "Bans",
        ...
        "Image Lookup": "Imagenes",
        "Translator": "Traductor",
        "Text to Speecjh": "Texto a Voz",
        "SONGS LYRICS": "Lyrics",
        "Greetings": "Bienvenidas/Despedidas",
        
#Some main stuff
"Here is the help for the *{}* module:\n{}": "Aquí está la ayuda para el módulo *{}*:\n{}",
"Back": "Atrás",
"send-start": """
Hola {}, me llamo {}! Si tienes alguna pregunta sobre como usarme, lee /help - y después ve a @NotAvaibleYet.
Soy un bot para administrar grupos mantenido por [esta maravillosa persona](t.me/CarlosBurelo). Soy parecida a [Marie](https://github.com/PaulSonOfLars/tgbot) 
...
[aquí](https://github.com/carlos-burelo/CortanaBot)!
...
Soy un bot de gestión de grupos modular con algunos extras divertidos! 
Echa un vistazo a lo siguiente para tener una idea de algunas de las cosas en las que puedo ayudarte.
...
Comandos principales disponibles:
 ...
 - /settings:
   - en privado: te enviará la configuración de todos los módulos disponibles.
   - en grupo: te redirecciona a un mensaje privado, con toda la configuración del grupo.
   {}
   """,
...
#Help modules
"Admin_help": """
 - /adminlist | /admins: lista los administradores de este chat
...
*Solo administradores:*
 - /ban <username>: banea a un usuario. (via nombre de usuario, o respondiendo a un mensaje suyo)
 - /tban <username> x(m/h/d): banea a un usuario durante x tiempo. (via nombre de usuario, o respondiendo a un mensaje suyo). m = minutos, h = horas, d = días.
 - /unban <username>: desbanea a un usuario. (via nombre de usuario, o respondiendo a un mensaje suyo)
 - /kick <username>: expulsa a un usuario, (via nombre de usuario, o respondiendo a un mensaje suyo)
""",
...
*Solo administradores:*
 - /mute <username>: silencia un usuario. Puede ser usado respondiendo a un mensaje, silenciando al usuario al que respondes.
 - /tmute <username> x(m/h/d): silencia a un usuario durante x tiempo.(via nombre de usuario, o respondiendo a un mensaje suyo). m = minutos, h = horas, d = días.
 - /unmute <username>: quita el silencio a un usuario. Puede ser usado como respuesta, quitando el silencio a usuario al que respondes.
 - /restrict <username>: restringe a un usuario para enviar stickers, gif, links o media. Puede ser usado como respuesta, restringiendo al usuario al que respondes.
 - /trestrict <username> x(m/h/d): restringe a un usuario durante x tiempo. (via nombre de usuario, o respondiendo a un mensaje suyo). m = minutos, h = horas, d = días.
 - /unrestrict <username>: quita la restricción a un usuario para enviar stickers, gif, links o media. Puede ser usado como respuesta, quitando la restricción al usuario al que respondes..
""",

"Notes_help": """
 - /get <notename>: obtienes la nota guardada
 - #<notename>: lo mismo que con /get
 - /notes o /saved: enlista todas las notas

Si quieres recuperar el contenido de una nota sin formato, utiliza 
- `/get <nombredenotenamelanota> noformat`. 
Esto puede \
ser util cuando actualizas una nota, sobre todo si hay botones en ella.

*Solo administradores:*
 - /save <notename> <content>: guarda 'content' como nota con el nombre 'notename'
...
`buttonurl:` tal como se muestra aquí: `[textodelbotón](buttonurl:tulink.com)`. Así quedaria configurado un botón en una nota. Mira /markdownhelp para más información.
 - /save <notename>: guarda el mensaje al que respondes con 'notename'
 - /clear <notename>: borra la nota con ese nombre
""",
...
"Warnings_help": """
 - /warns <@username>: te muestra el número de avisos y la razón del usuario al que respondes
 - /warnlist: lista de los filtros de avisos actual.

*Solo administradores:*
 - /warn <username>: advierte a un usuario. Despues de 3 advertencias, el usuario será baneado del grupo.
 - /resetwarn <username>: Resetea las advertencias para un usuario.
